"""Build a self-contained browser preview of the SHORT explainer: the animation stage + narration audio, in a player shell."""
...
opus = V / "out" / "mix_short.opus"
subprocess.run([FF, "-y", "-v", "error", "-i", str(V / "out" / "mix_short.wav"), "-ac", "1", "-ar", "48000", "-c:a", "libopus", "-b:a", "40k", str(opus)], check=True)
...
facts = [
 ("21.8 מיליארד ₪", "גיוסים לפוליסות חיסכון, מחצית ראשונה 2026 (+57% לעומת אשתקד)"),
...
lines = (S / "video" / "narration_short.txt").read_text().strip().splitlines()
script_html = "".join(f'<li>{l}</li>' for l in lines)

page = f"""<title>גיוסי פוליסות החיסכון 2026 - מקוצר</title>
...
<div class="wrap">
<header>
  <h1>הגיוסים לפוליסות החיסכון בשמיים (גרסה מקוצרת)</h1>
