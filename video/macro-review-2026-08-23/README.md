...
2. **`render_overlays.py`** draws one transparent 1080×1920 PNG per text beat
   (`storyboard.py` holds the copy, the timings and the palette). Hebrew is
   reordered for display with the bidi algorithm and every string is
   shrink-to-fit so nothing can run off the frame.
...
pip install Pillow numpy python-bidi
